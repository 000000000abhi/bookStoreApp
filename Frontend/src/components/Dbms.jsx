...
function Dbms() {
  ...
  // Filter free database courses
  const freeDatabaseCourses = book.filter(
    (item) =>  item.type === "free"
  );
    ...
    <div className="max-w-screen-2xl container mx-auto px-4 md:px-20 py-12">
      <div className="text-center mt-28">
        ...
        <p className="mt-6 text-gray-600 dark:text-gray-300">
          Explore our free database courses tailored to help you build a
          strong foundation in database management and systems. Our expert-led content
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {freeDatabaseCourses.length > 0 ? (
          freeDatabaseCourses.map((item) => (
            <Cards key={item.id} item={item} />
          ))
        ) : (
          <p className="text-center col-span-full text-gray-600 dark:text-gray-300">
            No free database courses available.
          </p>
        )}
      </div>
    </div>
